...
function buildBloodSugarLine() {
    var lastFourWeeks = new Date(Date.now() - 1000 * 3600 * 24 * 7 * 4);
    var lastTwoWeeks = new Date(Date.now() - 1000 * 3600 * 24 * 7 * 2);
    var bloodSugars = BloodSugar.find({userId: Meteor.userId(), updatedAt: {$gt: lastFourWeeks}}, {sort: {updatedAt: 1}}).fetch();
    var dataset = [];
    var bloodSugarsOnly = [];
    var datesOnly = [];
    
    bloodSugars.forEach(function(bloodSugar) {
      var item = {
        "date": bloodSugar.updatedAt,
...
//       };
      ...
      dataset.push(item);
      bloodSugarsOnly.push(bloodSugar.bloodsugar);
      datesOnly.push(moment(bloodSugar.updatedAt).format("DD/MM/YYYY"));
    });
  console.log(JSON.stringify(dataset));
  var bloodSugarsAsJSON = JSON.stringify(dataset);
  var bloodSugarsOnlyAsJSON = JSON.stringify(bloodSugarsOnly);  
  console.log(datesOnly);  
  console.log(JSON.stringify(dataset));
    $('#bloodSugarLine').highcharts({
        chart: {
            plotBackgroundColor: null,
            plotBorderWidth: null,
            plotShadow: false
...
//                 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
        ...
        series: [{
            name: Meteor.user().profile.firstName+'\'s Blood Sugar History',
            data: bloodSugarsOnly
        }]
    })
  }


Template.bloodSugarChart.rendered = function () {
