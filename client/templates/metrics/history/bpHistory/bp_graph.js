...
function buildBpLine() {
    var lastFourWeeks = new Date(Date.now() - 1000 * 3600 * 24 * 7 * 4);
    var lastTwoWeeks = new Date(Date.now() - 1000 * 3600 * 24 * 7 * 2);
    var bps = Weights.find({userId: Meteor.userId(), updatedAt: {$gt: lastFourWeeks}}, {sort: {updatedAt: 1}}).fetch();
    var dataset = [];
    var bpsOnly = [];
    var datesOnly = [];
...
//       };
      ...
      dataset.push(item);
      bpsOnly.push(bp.bloodpressure);
      datesOnly.push(moment(bp.updatedAt).format("DD/MM/YYYY"));
    });
  console.log(JSON.stringify(dataset));
  var bpsAsJSON = JSON.stringify(dataset);
  var bpsOnlyAsJSON = JSON.stringify(bpsOnly);  
  console.log(datesOnly);  
    $('#bpLine').highcharts({
        chart: {
            plotBackgroundColor: null,
        ...
        title: {
            text: Meteor.user().profile.firstName+'\'s Blood Pressure History',
            align: 'center',
            x: -20 //center
        },
        subtitle: {
...
//                 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
        ...
        tooltip: {
            valueSuffix: '\(Systolic\\Diastolic\)'
        },
        legend: {
            layout: 'vertical',
            align: 'center',
        ...
        series: [{
            name: Meteor.user().profile.firstName+'\'s Blood Pressure History',
            data: bpsOnly
        }]
    })
  }
...
Template.bpChart.rendered = function () {
